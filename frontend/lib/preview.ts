...
export function buildPreviewFromTwinPreview(
  ...
  return {
    ...
    lookingFor: backendLookingFor.length > 0 ? backendLookingFor : base.lookingFor,
    ...
    wantsHelpWith: backendLookingFor.slice(0, 3).length > 0 ? backendLookingFor.slice(0, 3) : base.wantsHelpWith,
    agentVoice: twin.agent_voice ?? base.agentVoice,
    privacySettings: getDefaultPrivacySettings(),
    completenessScore: twin.completeness_score ?? Math.min(95, 60 + filledFields * 5),
    avatarName: name,
    avatarInitials: persona?.avatarInitials,
    avatarColor: persona?.avatarColor,
  };
